...
class LightSpeech(nn.Module):
    """ RL-LightSpeech: LightSpeech combine with Reinforcement Learning """
    ...
    def mask(self, x, predicted, cemb, input_lengths, output_lengths, max_c_len, max_mel_len):
        ...
        p_mask = ~utils.get_mask_from_lengths(input_lengths, max_c_len)
        p_mask = p_mask.expand(hparams.P_dim, c_mask.size(0), c_mask.size(1))
        p_mask = p_mask.permute(1, 2, 0)

        x.data.masked_fill_(x_mask, 0.0)
        cemb.data.masked_fill_(c_mask, 0.0)
        predicted.data.masked_fill_(p_mask, 0.0)

        return x, predicted, cemb

    def forward(self, character, length_c, max_c_len, max_mel_len):
        ...
        # x, predicted, length_mel = self.LR(
        #     cemb, target=D, mel_max_length=max_mel_len)
        x, predicted, length_mel, history = self.LR(cemb,
                                                    length_c,
                                                    mel_max_length=max_mel_len)
        length_mel = torch.Tensor(length_mel).long().to(device)
